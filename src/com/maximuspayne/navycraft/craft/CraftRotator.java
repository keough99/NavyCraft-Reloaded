...
public class CraftRotator {
	...
	public void setBlock(double id, int X, int Y, int Z) {
		if (Y < 0 || Y > 255 || id < 0 || id > 255) return;
	...
	public boolean canMoveBlocks(int dx, int dy, int dz, int dr) {
		...
		// rotate dimensions
		Vector newSize = getCraftSize().clone();
		
		if (dr == 90 || dr == 270) {
			newSize.setX(getCraftSize().getZ());
			newSize.setZ(getCraftSize().getX());
		}
				...
				for (int y = 0; y < newSize.getBlockY(); y++) {
					...
					if (newMatrix[x][y][z] != 255) { // before move : craft block
						...
						if (getCraftBlockId(x + dx + newminX, y + dy, z + dz + newminZ, dr) == 255) {
							if (!canGoThrough(getWorldBlockId(x + dx, y + dy, z + dz, newRotation, newminX, craft.minY,
									newminZ, newoffsetX, newoffsetZ)))
								return false;
						}
					}
				}
	...
	public void turn(int dr) {
		...
		for (Entity e : craftEntities) {
			...
			int newoffsetX = (int) rotateX((double) (oldLoc.getBlockX() - (craft.minX + craft.offX)),
					(double) (oldLoc.getBlockZ() - (craft.minZ + craft.offZ)), dr);
			int newoffsetZ = (int) rotateZ((double) (oldLoc.getBlockX() - (craft.minX + craft.offX)),
					(double) (oldLoc.getBlockZ() - (craft.minZ + craft.offZ)), dr);
		...
		for (Entity e : craftEntities) {
			...
			// craft.player.sendMessage("tp coord=" + (double)(entPreLoc.get(e).getX() + craft.minX + craft.offX + 0.5)
			// + "," + (double)(entPreLoc.get(e).getZ() + craft.minZ + craft.offZ + 0.5) );
			
			e.teleport(new Location(craft.world, entPreLoc.get(e).getX() + craft.minX + craft.offX + 0.5,
					entPreLoc.get(e).getY(), entPreLoc.get(e).getZ() + craft.minZ + craft.offZ + 0.5,
				...
				for (int z = 0; z < newSize.getBlockZ(); z++) {
					...
					for (int i = 0; i < unMovedDataBlocks.size(); i++) {
						...
						if (dataBlock.locationMatches(newX, y, newZ)) {
							...
							/// check if cannon
							if (dataBlock.id == 23) {
								Location cannonLoc = new Location(craft.world, dataBlock.x + craft.minX,
										dataBlock.y + craft.minY, dataBlock.z + craft.minZ);
							...
							if (dataBlock.id == 19) {
								Location spongeLoc = new Location(craft.world, dataBlock.x + craft.minX,
										dataBlock.y + craft.minY, dataBlock.z + craft.minZ);
	...
	public void rotateCardinals(ArrayList<DataBlock> blocksToRotate, int dr) {
		...
		for (DataBlock dataBlock : blocksToRotate) {
			...
			//// stairs
			if (blockId == 53 || blockId == 67 || blockId == 108 || blockId == 109 || blockId == 114 || blockId == 128
					|| blockId == 134 || blockId == 135 || blockId == 136 || blockId == 156 || blockId == 180) {
				if (dataBlock.data > 3) { // upside down
					for (int c = 0; c < 4; c++) {
						cardinals[c] += 4;
					}
				}
			...
			if (blockId == 64 || blockId == 71 || blockId == 193 || blockId == 194 || blockId == 195 || blockId == 196
					|| blockId == 197// wooden or steel door
			...
			if (blockId == 77 || blockId == 143) { // button
				...
				if (dataBlock.data > 4) { // switched on
					for (int c = 0; c < 4; c++) {
						cardinals[c] += 8;
					}
				}
