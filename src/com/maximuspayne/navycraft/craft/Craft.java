...
public class Craft {
		...
		for (Craft craft : craftList) {
			if (craft.captainName != null) {
				...
				for (String s : craft.crewNames) {
					if (s.equalsIgnoreCase(player.getName())) // if( craft.isOnBoard.get(p) )
						return craft;
				}
			}
		}
	...
	public static Craft getCraft(int x, int y, int z) {
		...
		for (Craft craft : craftList) {
			if (craft.isIn(x, y, z)) return craft;
		}
		return null;
	}
	...
	public static Craft getOtherCraft(Craft c, Player p, int x, int y, int z) {
		...
		for (Craft craft : craftList) {
			if (craft.isIn(x, y, z) && getPlayerCraft(p) != craft) return craft;
		}
		return null;
	}
	...
	public boolean isOnCraft(Player player, boolean precise) {
		...
		if (isIn(x, y - 1, z)) {
			...
			if (matrix[x - minX][y - minY - 1][z - minZ] != -1) return true;
		}
	...
	public boolean isOnCraft(Entity player, boolean precise) {
		...
		if (isIn(x, y - 1, z)) {
			...
			// the block the player is standing on is part of the craft
			if (matrix[x - minZ][y - minY - 1][z - minZ] != -1) return true;
		}
	...
	public boolean isCraftBlock(int x, int y, int z) {
		...
		if (x >= 0 && y >= 0 && z >= 0 && x < sizeX && y < sizeY && z < sizeZ) return !(matrix[x][y][z] == -1);
		else return false;
	}
		...
		for (Chunk checkChunk : checkChunks) {
			if (!world.isChunkLoaded(checkChunk)) {
				try {
					...
					NavyCraft.instance
							.DebugMessage("Craft prevented from moving because destination chunk is not loaded.", 3);
					return false;
				}
			}
		}
	...
	public void buildCrew(Player newCaptain, boolean addOnly) {
			...
			if (e instanceof Player) {
				...
				if (p != newCaptain || type.canFly || type.isTerrestrial) {
					if (type.canFly) {
						...
						} else if (!p.getInventory().contains(Material.CHAINMAIL_HELMET)
						...
						} else if (!p.getInventory().contains(Material.CHAINMAIL_CHESTPLATE)
						...
						} else if (!p.getInventory().contains(Material.CHAINMAIL_LEGGINGS)
						...
						} else if (!p.getInventory().contains(Material.CHAINMAIL_BOOTS)
						...
						} else if (!p.getInventory().contains(Material.IRON_HELMET)
						...
						} else if (!p.getInventory().contains(Material.IRON_CHESTPLATE)
						...
						} else if (!p.getInventory().contains(Material.IRON_LEGGINGS)
						...
						} else if (!p.getInventory().contains(Material.IRON_BOOTS)
						...
						} else if (!p.getInventory().contains(Material.LEATHER_HELMET)
						...
						} else if (!p.getInventory().contains(Material.LEATHER_CHESTPLATE)
						...
						} else if (!p.getInventory().contains(Material.LEATHER_LEGGINGS)
						...
						} else if (!p.getInventory().contains(Material.LEATHER_BOOTS)
		...
		if (!type.canFly && !type.isTerrestrial) {
			...
			} else if (!captain.getInventory().contains(Material.GOLD_HELMET)
					&& captain.getInventory().getHelmet().getType() != Material.GOLD_HELMET) {
			...
			} else if (!captain.getInventory().contains(Material.GOLD_CHESTPLATE)
					&& captain.getInventory().getChestplate().getType() != Material.GOLD_CHESTPLATE) {
			...
			} else if (!captain.getInventory().contains(Material.GOLD_LEGGINGS)
					&& captain.getInventory().getLeggings().getType() != Material.GOLD_LEGGINGS) {
			...
			} else if (!captain.getInventory().contains(Material.GOLD_BOOTS)
					&& captain.getInventory().getBoots().getType() != Material.GOLD_BOOTS) {
		...
		if (customName == null) {
			for (String s : crewNames) {
				...
				if (p != null && s != captainName) {
					p.sendMessage(ChatColor.GOLD + "You join the crew of the " + ChatColor.WHITE + name.toUpperCase()
							+ ChatColor.GOLD + " class under the command of " + newCaptain.getDisplayName() + "!");
			...
			for (String s : crewNames) {
				Player p = NavyCraft.instance.getServer().getPlayer(s);
				if (p != null && s != captainName) {
					p.sendMessage(ChatColor.GOLD + "You join the " + ChatColor.WHITE + name.toUpperCase()
							+ ChatColor.YELLOW + " under the command of " + newCaptain.getDisplayName() + "!");
					newCaptain.sendMessage(p.getDisplayName() + ChatColor.GOLD + " joins your crew!");
				}
			}
		}
	...
	public int[] enginesByEngineFace(CraftMover cm) {
		...
		for (DataBlock edb : engineBlocks) {
			...
			int engineDirection = BlocksInfo.getCardinalDirectionFromData(engineBlock.getTypeId(),
					engineBlock.getData());
	...
	public static int blockHardness(int blockID) {
		if (blockID == 7 || blockID == 120 || blockID == 130 || blockID == 137) return 4;
		else if (blockID == 49 || blockID == 116 || blockID == 145) return 3;
		else if (blockID == 8 || blockID == 9 || blockID == 10 || blockID == 11) return -3;
		else if (blockID == 1 || blockID == 4 || blockID == 23 || blockID == 41 || blockID == 251 || blockID == 42
				|| blockID == 48 || blockID == 57
				|| blockID == 67 || blockID == 71 || blockID == 86 || blockID == 98 || blockID == 109 || blockID == 112
				|| blockID == 114 || blockID == 121 || blockID == 122 || blockID == 133 || blockID == 139
				|| blockID == 155
				|| blockID == 156 || blockID == 251 || blockID >= 235 && blockID <= 250)
			return 2;
		else if (blockID == 2 || blockID == 3 || blockID == 5 || blockID == 12 || blockID == 13 || blockID == 14
				|| blockID == 15 || blockID == 16
				|| blockID == 17 || blockID == 19 || blockID == 20 || blockID == 95 || blockID == 21 || blockID == 22
				|| blockID == 24 || blockID == 25 || blockID == 29 || blockID == 33 || blockID == 35
				|| blockID == 43 || blockID == 44 || blockID == 45 || blockID == 47 || blockID == 53 || blockID == 54
				|| blockID == 56 || blockID == 58 || blockID == 60 || blockID == 61
				|| blockID == 62 || blockID == 63 || blockID == 64 || blockID == 65 || blockID == 68 || blockID == 69
				|| blockID == 70 || blockID == 72 || blockID == 73 || blockID == 74
				|| blockID == 77 || blockID == 78 || blockID == 79 || blockID == 80 || blockID == 82 || blockID == 84
				|| blockID == 85 || blockID == 91 || blockID == 96 || blockID == 101 || blockID == 107
				|| blockID == 108 || blockID == 110 || blockID == 113 || blockID == 117 || blockID == 118
				|| blockID == 125 || blockID == 126
				|| blockID == 128 || blockID == 159 || blockID == 172 || blockID == 134 || blockID == 135
				|| blockID == 136 || blockID == 138 || blockID == 143 || blockID == 146 || blockID == 147
				|| blockID == 148 || blockID == 154 || blockID == 158)
			return 1;
		else if (blockID == 46 || blockID == 129 || blockID == 152) return -1;
		else if (blockID == 153 || blockID == 170 || blockID == 173) return -2;
		else return 0;
	}

	public static float blockWeight(int blockID) {
		if (blockID == 7 || blockID == 49 || blockID == 130) return 20.0f;
		else if (blockID == 1 || blockID == 2 || blockID == 3 || blockID == 4 || blockID == 12 || blockID == 13
				|| blockID == 22 || blockID == 23
				|| blockID == 24 || blockID == 29 || blockID == 33 || blockID == 41 || blockID == 42 || blockID == 45
				|| blockID == 46 || blockID == 48 || blockID == 57 || blockID == 60 || blockID == 61 || blockID == 62
				|| blockID == 67 || blockID == 71 || blockID == 78 || blockID == 79 || blockID == 80 || blockID == 82
				|| blockID == 86 || blockID == 98
				|| blockID == 108 || blockID == 109 || blockID == 110 || blockID == 112 || blockID == 114
				|| blockID == 116 || blockID == 120 || blockID == 121 || blockID == 128 || blockID == 129
				|| blockID == 133
				|| blockID == 138 || blockID == 139 || blockID == 145 || blockID == 152 || blockID == 154
				|| blockID == 155 || blockID == 156 || blockID == 158 || blockID == 251
				|| blockID >= 235 && blockID <= 250)
			return 1.0f;
		else if (blockID == 19 || blockID == 43 || blockID == 54 || blockID == 58 || blockID == 64 || blockID == 91
				|| blockID == 96 || blockID == 101 || blockID == 113
				|| blockID == 117 || blockID == 118 || blockID == 137 || blockID == 153 || blockID == 170
				|| blockID == 173 || blockID == 172)
			return 0.5f;
		else if (blockID == 5 || blockID == 14 || blockID == 15 || blockID == 16 || blockID == 17 || blockID == 21
				|| blockID == 25 || blockID == 44 || blockID == 47
				|| blockID == 53 || blockID == 56 || blockID == 63 || blockID == 65 || blockID == 68 || blockID == 73
				|| blockID == 74 || blockID == 159 || blockID == 84 || blockID == 125 || blockID == 126
				|| blockID == 134 || blockID == 135 || blockID == 136)
			return 0.25f;
		else if (blockID == 35 || blockID == 85 || blockID == 107) return 0.17f;
		else if (blockID != 0 && (blockID < 8 || blockID > 11)) return 0.1f;
		else return 0.0f;
	}

	public static boolean getAttachedBlockExists(Block inBlock, int blockID, int data) {
		if (getAttachedBlock(inBlock, blockID, data).getTypeId() == 0
				|| getAttachedBlock(inBlock, blockID, data).getTypeId() >= 8
						&& getAttachedBlock(inBlock, blockID, data).getTypeId() <= 11)
			return false;
		else
			return true;
	}
	...
	public static Block getAttachedBlock(Block inBlock, int blockID, int data) {
		...
		} else if (blockID == 65 || blockID == 68) // ladder and wallsign
		{
			if (data == 2)
				...
				return inBlock.getRelative(BlockFace.WEST);
		} else if (blockID == 63) return inBlock.getRelative(BlockFace.DOWN);
		else if (blockID == 69 || blockID == 77 || blockID == 143) // lever and buttons
		{
			if (data % 8 == 0)
				...
				return inBlock.getRelative(BlockFace.UP);
		} else if (blockID == 70 || blockID == 72 || blockID == 55 || blockID == 64 || blockID == 71)
			return inBlock.getRelative(BlockFace.DOWN);
		return null;
	}
	...
	public void speedChange(Player player, boolean increase) {
		...
		if (increase) {
			...
			if (!type.canFly) {
				setSpeed = setSpeed - 1;
				if (setSpeed <= 0) {
					if (NavyCraft.checkSpawnRegion(new Location(world, minX, minY, minZ))
							|| NavyCraft.checkSpawnRegion(new Location(world, maxX, maxY, maxZ))) {
						if (player != null) {
		...
		if (player != null) {
			if (type.canFly) {
				player.sendMessage(
						ChatColor.GOLD + "Throttle-" + ChatColor.GREEN + setSpeed * 10 + ChatColor.GOLD + "%");
			} else if (type.isTerrestrial) {
				player.sendMessage(
						ChatColor.GOLD + "Throttle-" + ChatColor.GREEN + setSpeed * 25 + ChatColor.GOLD + "%");
			} else {
				if (setSpeed == 0) {
					...
					player.sendMessage(ChatColor.GOLD + "Engines " + ChatColor.GREEN + "1" + ChatColor.DARK_GRAY + "/"
							+ ChatColor.RED + "3");
				} else if (setSpeed == 3) {
					player.sendMessage(ChatColor.GOLD + "Engines " + ChatColor.GREEN + "2" + ChatColor.DARK_GRAY + "/"
							+ ChatColor.RED + "3");
				} else if (setSpeed == 4) {
	...
	public void gearChange(Player player, boolean increase) {
		...
		if (player != null) {
			player.sendMessage(ChatColor.GOLD + "Set engines to Gear" + ChatColor.DARK_GRAY + " - " + ChatColor.GREEN
					+ gear + ChatColor.DARK_GRAY + "/" + ChatColor.RED + type.maxForwardGear);
		}
	}
	...
	public void rudderChange(Player player, int order, boolean turn) {
		...
		if (order == 1) {
			if (rudder == 0 || rudder == 1 && turn && turnProgress == 0) {
				...
				if (player != null) {
					player.sendMessage(
							ChatColor.RED + "Rudder already set. " + ChatColor.GOLD + "Look other way to cancel.");
				}
			}
		} else if (order == -1) {
			if (rudder == 0 || rudder == -1 && turn && turnProgress == 0) {
			...
			if (i < 23) {
				...
				if (craft.abandoned && craft.isOnCraft(player, false) && !craft.crewNames.contains(player.getName())) {
					if (craft.customName != null) {
						if (craft.captainName != null) {
							plugin.getServer().broadcastMessage(ChatColor.WHITE + craft.captainName + ChatColor.GOLD
									+ "'s " + ChatColor.WHITE + craft.customName + ChatColor.GOLD + " was abandoned.");
						} else {
							plugin.getServer().broadcastMessage(ChatColor.GOLD + "The " + ChatColor.WHITE
									+ craft.customName + ChatColor.GOLD + " was abandoned.");
						}
					} else {
						if (craft.captainName != null) {
							plugin.getServer().broadcastMessage(ChatColor.WHITE + craft.captainName + ChatColor.GOLD
									+ "'s " + ChatColor.WHITE + craft.name + ChatColor.GOLD + " was abandoned.");
						} else {
							plugin.getServer().broadcastMessage(ChatColor.GOLD + "The " + ChatColor.WHITE + craft.name
									+ ChatColor.GOLD + " was abandoned.");
						}
					}
