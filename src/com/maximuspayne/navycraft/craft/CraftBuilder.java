...
public class CraftBuilder {
	...
	private static Short get(int x, int y, int z) {
		...
		if (xRow != null) {
			...
			if (yRow != null) return yRow.get(new Integer(z));
		}
	...
	private static boolean secondPassDetection() {
			...
			for (int z = 0; z < craft.sizeZ; z++) {
				...
				for (int y = 0; y < craft.sizeY; y++) {
					...
					if (!floor && craft.matrix[x][y][z] != -1) {
						...
						craft.matrix[x][y][z] = (short) blockId; // record this block as part of the craft, also fill
																	// with air
		...
		if (craft.type.canDive || craft.type.canNavigate) {
			...
			blocksStack.push(new BlockLoc(
					(int) Math.floor(NavyCraft.instance.getServer().getPlayer(craft.captainName).getLocation().getX())
							- craft.minX,
					(int) Math.floor(NavyCraft.instance.getServer().getPlayer(craft.captainName).getLocation().getY()
							+ 1 - craft.minY),
					(int) Math.floor(NavyCraft.instance.getServer().getPlayer(craft.captainName).getLocation().getZ())
							- craft.minZ));
			
			// detect all connected empty blocks
			do {
				if (!createAirBubble()
						&& NavyCraft.instance.getConfig().getString("AllowHoles").equalsIgnoreCase("false")
						&& craft.type.canDive) {
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(
							ChatColor.RED + "This " + craft.type.name + " has holes, it needs to be waterproof");
					return false;
				}
	...
	private static void createMatrix() {
		...
		if (craft.sizeX <= 1 || craft.sizeY <= 1 || craft.sizeZ <= 1) return;
		craft.matrix = new short[craft.sizeX][craft.sizeY][craft.sizeZ];
		craft.displacedBlocks = new short[craft.matrix[0].length + 1][craft.matrix[1].length + 1][craft.matrix[2].length
				+ 1];
	...
	private static void detectBlock(int x, int y, int z, int dir) {
		...
		if (craft.type.forbiddenBlocks != null && craft.type.forbiddenBlocks.length > 0
			...
			for (short forbiddenBlock : craft.type.forbiddenBlocks) {
				if (blockType == forbiddenBlock) {
					NavyCraft.instance.getServer().getPlayer(craft.captainName)
							.sendMessage("Forbidden block of type " + Material.getMaterial(blockType) + " found. " +
		...
		if (craft.blockCount > craft.type.maxBlocks) return;
	...
	public static boolean detect(Craft craft, int X, int Y, int Z) {
		...
		if (craft.blockCount > craft.type.maxBlocks) {
			NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(
					ChatColor.RED + "Unable to detect the " + craft.name + ", be sure it is not connected");
			NavyCraft.instance.getServer().getPlayer(craft.captainName)
					.sendMessage(ChatColor.RED + " to the ground, or maybe it is too big for this type of craft");
			NavyCraft.instance.getServer().getPlayer(craft.captainName)
					.sendMessage(ChatColor.RED + "The maximum size is " + craft.type.maxBlocks + " blocks");
			return false;
		} else if (craft.blockCount < craft.type.minBlocks) {
			if (craft.blockCount == 0) {
				NavyCraft.instance.getServer().getPlayer(craft.captainName)
						.sendMessage(ChatColor.RED + "There is no " + craft.name + " here");
				NavyCraft.instance.getServer().getPlayer(craft.captainName)
						.sendMessage(ChatColor.RED + "Be sure you are standing on a block");
			} else {
				NavyCraft.instance.getServer().getPlayer(craft.captainName)
						.sendMessage(ChatColor.RED + "This " + craft.name + " is too small !");
				NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(
						ChatColor.RED + "You need to add " + (craft.type.minBlocks - craft.blockCount) + " blocks");
			}
		...
		// the recursive algorithm returned before the max block count have been reached, we have a craft !
				...
				if (c != craft && c.captainName != null && !c.launcherOn) {
					...
					if (!(c.minX < craft.minX && c.maxX < craft.minX || craft.minX < c.minX && craft.maxX < c.minX))
						if (!(c.minY < craft.minY && c.maxY < craft.minY || craft.minY < c.minY && craft.maxY < c.minY))
							if (!(c.minZ < craft.minZ && c.maxZ < craft.minZ
									|| craft.minZ < c.minZ && craft.maxZ < c.minZ)) {
							NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(
									ChatColor.RED + "" + c.captainName + " is already controling this " + craft.name);
							return false;
							}
				}
			}
			...
			if (craft.type.canNavigate && !craft.type.canFly && craft.waterType == 0 && !craft.type.canDig
					&& !craft.type.isTerrestrial) {
				NavyCraft.instance.getServer().getPlayer(craft.captainName)
						.sendMessage(ChatColor.RED + "This " + craft.name + " is not on water!");
				return false;
			} else
			
			if (craft.type.canDive && !craft.type.canFly && craft.waterType == 0 && !craft.type.canDig) {
				NavyCraft.instance.getServer().getPlayer(craft.captainName)
						.sendMessage(ChatColor.RED + "This " + craft.name + " is not into water!");
				return false;
			} else
			// the airplane / airship is into water
			if (craft.type.canFly && !craft.type.canNavigate && !craft.type.canDive && craft.waterLevel > -1) {
				NavyCraft.instance.getServer().getPlayer(craft.captainName)
						.sendMessage(ChatColor.RED + "This " + craft.name + " is into water!");
				return false;
			}
			 ...
			 * //int flyBlocksNeeded = (int)Math.floor((blockCount - flyBlockCount) * ((float)type.flyBlockPercent *
			 * 0.01));
			 * //let's hope it is correct :P
			 * int flyBlocksNeeded = (int)Math.floor((craft.blockCount - craft.flyBlockCount) *
			 * ((float)craft.type.flyBlockPercent * 0.01) / (1 - ((float)craft.type.flyBlockPercent * 0.01)));
			 * if(flyBlocksNeeded < 1)
			 * flyBlocksNeeded = 1;
			 * if(craft.flyBlockCount < flyBlocksNeeded){
			 * NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED + "Not enough " +
			 * BlocksInfo.getName(craft.type.flyBlockType) + " to make this " + craft.name + " move");
			 * NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(ChatColor.RED +
			 * "You need to add " + (flyBlocksNeeded - craft.flyBlockCount) + " more" );
			 * return false;
			 * }
			 * }
			 */
			...
			if (craft.type.canDig && craft.type.digBlockId != 0) {
				...
				int digBlocksNeeded = (int) Math
						.floor((craft.blockCount - craft.digBlockCount) * ((float) craft.type.digBlockPercent * 0.01)
								/ (1 - (float) craft.type.digBlockPercent * 0.01));
				...
				if (craft.digBlockCount < digBlocksNeeded) {
					NavyCraft.instance.getServer().getPlayer(craft.captainName)
							.sendMessage(ChatColor.RED + "Not enough " + BlocksInfo.getName(craft.type.digBlockId)
									+ " to make this " + craft.name + " move");
					NavyCraft.instance.getServer().getPlayer(craft.captainName).sendMessage(
							ChatColor.RED + "You need to add " + (digBlocksNeeded - craft.digBlockCount) + " more");
					return false;
				}
			}
			{
				...
				if (craft.customName != null) {
					captain.sendMessage(ChatColor.GOLD + "You Launch the " + ChatColor.WHITE
							+ craft.customName.toUpperCase() + ChatColor.GOLD + " class!");
				} else {
					captain.sendMessage(ChatColor.GOLD + "You Launch the " + ChatColor.WHITE + craft.name.toUpperCase()
							+ ChatColor.GOLD + " class!");
				}
			}
		}
