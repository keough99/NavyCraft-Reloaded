...
public class NavyCraft extends JavaPlugin {
	...
	public Craft createCraft(Player player, CraftType craftType, int x, int y, int z, String name, float dr,
			Block signBlock) {
		...
		if (!CraftBuilder.detect(craft, x, y, z)) return null;
		...
		if (checkSpawnRegion(new Location(craft.world, craft.minX, craft.minY, craft.minZ))
		...
		if (shutDown = false || PermissionInterface.CheckEnabledWorld(new Location(craft.world, craft.minX, craft.minY, craft.minZ))){
			...
			if (craft.type.listenItem == true) {
				player.sendMessage(ChatColor.GRAY
						+ "With a gold sword in your hand, right-click in the direction you want to go.");
			}
	...
	public static int checkTeamRegion(Location loc) /// 0 no region, 1 blue team, 2 red team
	{
		...
		if (wgp != null && loc != null) {
			if (!loc.getWorld().getName().equalsIgnoreCase("warworld1")
					&& !loc.getWorld().getName().equalsIgnoreCase("warworld2")
					&& !loc.getWorld().getName().equalsIgnoreCase("warworld3"))
				return 0;
	...
	public static boolean checkRepairRegion(Location loc) {
		...
		wgp = (WorldGuardPlugin) instance.getServer().getPluginManager().getPlugin("WorldGuard");
		if (wgp != null && loc != null) {
			if (!!PermissionInterface.CheckEnabledWorld(loc)) return false;
			RegionManager regionManager = wgp.getRegionManager(loc.getWorld());
				...
				if (checkCraft.isMoving) {
					...
					if (updateNum == 0) {
						if (checkCraft.doRemove) {
							...
							if (Math.abs(checkCraft.gear) == 1
									&& (System.currentTimeMillis() - checkCraft.lastMove) / 1000 > 8
									|| Math.abs(checkCraft.gear) == 2
											&& (System.currentTimeMillis() - checkCraft.lastMove) / 1000 >= 5
									|| Math.abs(checkCraft.gear) == 3
											&& (System.currentTimeMillis() - checkCraft.lastMove) / 1000.0f >= 2.5) {
						...
						if (doPower) {
							...
							if (refPower > 0) {
								...
								if (Craft.blockHardness(blockType) == -1) {
									...
									TNTPrimed tnt = (TNTPrimed) theBlock.getWorld()
											.spawnEntity(new Location(theBlock.getWorld(), theBlock.getX(),
													theBlock.getY(), theBlock.getZ()), EntityType.PRIMED_TNT);
