...
public class NavyCraft_EntityListener implements Listener {
		...
		if (ent != null && ent instanceof TNTPrimed) {
			if (event.getLocation() != null) {
				if (NavyCraft.shotTNTList.containsKey(ent.getUniqueId())) {
					...
					if (checkCraft == null) {
						checkCraft = structureUpdate(event.getLocation().getBlock().getRelative(4, 4, 4).getLocation(),
								NavyCraft.shotTNTList.get(ent.getUniqueId()));
						if (checkCraft == null) {
							checkCraft = structureUpdate(
									event.getLocation().getBlock().getRelative(-4, -4, -4).getLocation(),
									NavyCraft.shotTNTList.get(ent.getUniqueId()));
							if (checkCraft == null) {
								checkCraft = structureUpdate(
										event.getLocation().getBlock().getRelative(2, -2, -2).getLocation(),
										NavyCraft.shotTNTList.get(ent.getUniqueId()));
								if (checkCraft == null) {
									checkCraft = structureUpdate(
											event.getLocation().getBlock().getRelative(-2, 2, 2).getLocation(),
	...
	public void onEntityTarget(EntityTargetEvent event) {
		if (event.getEntity() instanceof Skeleton && NavyCraft.aaSkelesList.contains(event.getEntity())) {
			if (event.getTarget() instanceof Player) {
				Player target = (Player) event.getTarget();
				Craft skeleCraft = Craft.getCraft(target.getLocation().getBlockX(), target.getLocation().getBlockY(),
						target.getLocation().getBlockZ());
				if (skeleCraft != null && !skeleCraft.crewNames.isEmpty()
						&& skeleCraft.crewNames.contains(target.getName())) {
	...
	public void onEntityDamage(EntityDamageEvent event) {
		...
		if (event.getEntity() instanceof Player) {
			...
			if (event.getCause() == DamageCause.SUFFOCATION) {
				Craft c = Craft.getCraft(player.getLocation().getBlockX(), player.getLocation().getBlockY(),
		...
		if (event instanceof EntityDamageByEntityEvent) {
			...
			if (attacker instanceof Egg) {
				if (NavyCraft.explosiveEggsList.contains(attacker)) {
					event.setDamage(8);
				}
			}
		}
