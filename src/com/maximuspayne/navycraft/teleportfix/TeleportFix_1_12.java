		...
		if (oldLight != null) {
			...
			for (Player p : oc.getWorld().getPlayers()) {
				...
				for (int i = -3; i <= 3; i++) {
					for (int j = -3; j <= 3; j++) {
						for (int k = -3; k <= 3; k++) {
							Location loc = new Location(block.getWorld(), block.getX() + i, block.getY() + j,
									block.getZ() + k);
		...
		if (light != null) {
			...
			for (Player p : c.getWorld().getPlayers()) {
				...
				for (int i = -3; i <= 3; i++) {
					for (int j = -3; j <= 3; j++) {
						for (int k = -3; k <= 3; k++) {
							int strength = (int) (15 - Math.sqrt(
									Math.abs(i) * Math.abs(i) + Math.abs(j) * Math.abs(j) + Math.abs(k) * Math.abs(k))
									* 3);
							Location loc = new Location(block.getWorld(), block.getX() + i, block.getY() + j,
									block.getZ() + k);
							nmsChunk = ((CraftChunk) loc.getChunk()).getHandle();
