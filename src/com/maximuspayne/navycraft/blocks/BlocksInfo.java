...
public class BlocksInfo {
	...
	public static void loadBlocksInfo() {
		...
		// name, isDataBlock, needsSupport, isComplexBlock, item dropped, amount dropped, is grass cover, cardinal
			// directions
		...
		blocks[61] = new BlockInfo(61, "furnace", false, true, 4, 8, new byte[] { 4, 2, 5, 3 }); /*
																									 * Might need
																									 * support...
																									 */
	...
	public static int getCardinalDirectionFromData(int BlockId, short BlockData) {
		...
		for (int i = 0; i < blocks[BlockId].cardinalDirections.length; i++) {
			if (BlockData == blocks[BlockId].cardinalDirections[i]) return i;
		}
		return -1;
	}
	...
	public static class BlockInfo {
		...
		/* Given grasscover */
		private BlockInfo(int id, String name, boolean isDataBlock, boolean needSupport, boolean isComplexBlock,
				boolean isGrassCover) {
		...
		private BlockInfo(int id, String name, boolean isDataBlock, boolean isComplexBlock, int dropItem,
				int dropQuantity, byte[] cardinals) {
