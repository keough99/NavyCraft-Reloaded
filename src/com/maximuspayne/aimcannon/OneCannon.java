...
public class OneCannon {
	...
	boolean ignite;
	int cannonLength;
	public int cannonType; // 0 single, 1 double, 2 fireball (crap), 3 torpedo mk2, 4 depth charge, 5 depth charger II,
							// 6 triple cannon, 7 torp mk3, 8 torp mk1, 9 bombs
	int delay;
	int range;
	int torpedoMode;
	int depth;
	...
	public boolean isTimeout() {
		if (timeout + 4000 < new Date().getTime()) return true;
		else return false;
	...
	public void Ignite(final Player p) {
		...
		if (cannonType == 0) {
			...
			if (b.getType() == Material.AIR) {
				...
				tntp = (TNTPrimed) b.getWorld().spawnEntity(
						new Location(b.getWorld(), b.getX() + 0.5, b.getY() + 0.5, b.getZ() + 0.5),
			...
			if (b.getType() == Material.AIR) {
				...
				tntp = (TNTPrimed) c.getWorld().spawnEntity(
						new Location(c.getWorld(), c.getX() + 0.5, c.getY() + 0.5, c.getZ() + 0.5),
						EntityType.PRIMED_TNT);
				
				tntp2 = (TNTPrimed) d.getWorld().spawnEntity(
						new Location(d.getWorld(), d.getX() + 0.5, d.getY() + 0.5, d.getZ() + 0.5),
						EntityType.PRIMED_TNT);
				NavyCraft.shotTNTList.put(tntp.getUniqueId(), p);
			...
			if (b.getType() == Material.AIR) {
				...
				tntp3 = (TNTPrimed) b.getWorld().spawnEntity(
						new Location(b.getWorld(), b.getX() + 0.5, b.getY() + 0.5, b.getZ() + 0.5),
						EntityType.PRIMED_TNT);
	...
	public void fire1(final Player p) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> p.sendMessage("2 - Aim!"));
	}
	
	public void fire2(final Player p) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> p.sendMessage("1 - Fire!!!"));
	}
	
	public void fire3(final Player p) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
			Vector look;
		...
		if (cannonType == 6) {
			if (direction == BlockFace.EAST || direction == BlockFace.WEST) {
				b.getWorld().createExplosion(
						loc.getBlock().getRelative(direction, 4).getRelative(BlockFace.NORTH, 2).getLocation(), 0);
				b.getWorld().createExplosion(
						loc.getBlock().getRelative(direction, 4).getRelative(BlockFace.SOUTH, 2).getLocation(), 0);
			} else {
				b.getWorld().createExplosion(
						loc.getBlock().getRelative(direction, 4).getRelative(BlockFace.EAST, 2).getLocation(), 0);
				b.getWorld().createExplosion(
						loc.getBlock().getRelative(direction, 4).getRelative(BlockFace.WEST, 2).getLocation(), 0);
			}
		}
	}
	...
	public void fireCannonUpdate(final Block b, final double x2, final double y2, final double z2, final double x1,
			final double y1, final double z1, final int i) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
		...
		if (!leftClick && (cannonType == 3 || cannonType == 7 || cannonType == 8)) {
			...
			if (cannonType == 0) {
				...
				if (charged == 0) {
					...
					p.sendMessage(
							"Depth charge launcher already loaded! " + ammunition + " depth charge launches left.");
				}
	...
	public void loadingTorpUpdate(final boolean left, final Block b, final BlockFace torpHeading, final boolean lift,
			final int i) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
			Block warhead = b;
			Craft testCraft = Craft.getCraft(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
			...
			if (checkInnerDoorClosed(leftInner)) {
				if (checkOuterDoorClosed()) {
					...
					if (leftInner) {
						...
						if (b.getRelative(direction, 2).getTypeId() >= 8
								&& b.getRelative(direction, 2).getTypeId() <= 11) {
							b.getRelative(direction, 2).setType(Material.AIR);
						}
						if (b.getRelative(direction, 3).getTypeId() >= 8
								&& b.getRelative(direction, 3).getTypeId() <= 11) {
							b.getRelative(direction, 3).setType(Material.AIR);
						}
						if (b.getRelative(direction, 4).getTypeId() >= 8
								&& b.getRelative(direction, 4).getTypeId() <= 11) {
						...
						if (c.getRelative(direction, 2).getTypeId() >= 8
								&& c.getRelative(direction, 2).getTypeId() <= 11) {
							c.getRelative(direction, 2).setType(Material.AIR);
						}
						if (c.getRelative(direction, 3).getTypeId() >= 8
								&& c.getRelative(direction, 3).getTypeId() <= 11) {
							c.getRelative(direction, 3).setType(Material.AIR);
						}
						if (c.getRelative(direction, 4).getTypeId() >= 8
								&& c.getRelative(direction, 4).getTypeId() <= 11) {
	...
	public void fireTorpedoMk1(final Player p, final Block b, final BlockFace torpHeading, final int torpDepth,
			final int delayShoot, final boolean left) {
	...
	public void fireTorpedoUpdateMk1(final Player p, final Weapon torp, final int i, final Craft firingCraft,
			final boolean left) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
			
			if (!torp.dead) {
				if (torp.warhead.getTypeId() == 35 && torp.warhead.getRelative(torp.hdg, -1).getTypeId() == 35
						&& torp.warhead.getRelative(torp.hdg, -2).getTypeId() == 35
						&& torp.warhead.getRelative(torp.hdg, -3).getTypeId() == 35) {
					...
					if (i > 15) {
						...
						// check new position
						if (torp.warhead.getType() == Material.WATER || torp.warhead.getType() == Material.AIR
								|| torp.warhead.getType() == Material.STATIONARY_WATER) {
							if (i == 149) {
								if (torp.warhead.getY() > 62) {
									torp.warhead.setType(Material.AIR);
						...
						} else if (torp.active) /// detonate!
						{
							...
							Craft checkCraft1 = null;
							checkCraft1 = NavyCraft.instance.entityListener
									.structureUpdate(torp.warhead.getRelative(torp.hdg, 2).getLocation(), p);
							if (checkCraft1 == null) {
								checkCraft1 = NavyCraft.instance.entityListener
										.structureUpdate(torp.warhead.getRelative(7, 7, 7).getLocation(), p);
								if (checkCraft1 == null) {
									checkCraft1 = NavyCraft.instance.entityListener
											.structureUpdate(torp.warhead.getRelative(-7, -7, -7).getLocation(), p);
									if (checkCraft1 == null) {
										checkCraft1 = NavyCraft.instance.entityListener
												.structureUpdate(torp.warhead.getRelative(3, -2, -3).getLocation(), p);
										if (checkCraft1 == null) {
											checkCraft1 = NavyCraft.instance.entityListener.structureUpdate(
													torp.warhead.getRelative(-3, 2, 3).getLocation(), p);
										}
									}
								}
							}
							...
							if (checkCraft1 == null) {
								...
								p.sendMessage("Torpedo hit " + checkCraft1.name + "!");
							}
				...
				} else // else torp blocks missing, detonate
				{
					...
					Craft checkCraft2 = null;
					checkCraft2 = NavyCraft.instance.entityListener
							.structureUpdate(torp.warhead.getRelative(torp.hdg, 2).getLocation(), p);
					if (checkCraft2 == null) {
						checkCraft2 = NavyCraft.instance.entityListener
								.structureUpdate(torp.warhead.getRelative(7, 7, 7).getLocation(), p);
					...
					if (checkCraft2 == null) {
						...
						p.sendMessage("Torpedo hit " + checkCraft2.name + "!");
					}
	...
	public void fireTorpedoMk2(final Player p, final Block b, final BlockFace torpHeading, final int torpDepth,
			final int delayShoot, final boolean left) {
		// final int taskNum;
	...
	public void fireTorpedoUpdateMk2(final Player p, final Weapon torp, final int i, final Craft firingCraft,
			final boolean left) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
	...
	public void fireTorpedoMk3(final Player p, final Block b, final BlockFace torpHeading, final int torpDepth,
			final int delayShoot, final boolean left) {
		// final int taskNum;
		// int taskNum = plugin.getServer().getScheduler().scheduleAsyncDelayedTask(plugin, new Runnable(){
		final Craft testCraft = Craft.getCraft(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
		...
		if (testCraft != null && torp.tubeNum != 0 && testCraft.tubeFiringMode.containsKey(torp.tubeNum)) {
			...
			// if( testCraft.tubeFiringMode.get(torp.tubeNum) == -1 && (onScopePlayer != null ||
			// testCraft.lastPeriscopeYaw != -9999) )
			// {
			/*
			 * float rotation=0;
	...
	public void fireTorpedoUpdateMk3(final Player p, final Weapon torp, final int i, final Craft firingCraft,
			final boolean left) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
				...
				if (torp.warhead.getTypeId() == 35 && torp.warhead.getRelative(torp.hdg, -1).getTypeId() == 35
					...
					if (i > 15) {
						...
						if (firingCraft != null && firingCraft.tubeFiringMode.containsKey(torp.tubeNum)) {
							...
							if (!firingCraft.tubeFiringArmed.get(torp.tubeNum)
									&& (i == firingCraft.tubeFiringArm.get(torp.tubeNum)
											|| i == 11 && firingCraft.tubeFiringArm.get(torp.tubeNum) == 10)) {
	...
	public boolean checkOuterDoorClosed() {
		...
		if (b.getRelative(BlockFace.EAST).getType() == Material.CLAY
				&& b.getRelative(BlockFace.WEST).getType() == Material.CLAY)
			return true;
		else if (b.getRelative(BlockFace.NORTH).getType() == Material.CLAY
				&& b.getRelative(BlockFace.SOUTH).getType() == Material.CLAY)
			return true;
		else return false;
	}
	...
	public boolean isValidCannon(Block b) {
		...
		if (b.getRelative(BlockFace.NORTH, 1).getType() == Material.DIAMOND_BLOCK) {
			...
			if (direction == null) {
				if (b.getRelative(BlockFace.NORTH, 1).getType() == Material.WOOD
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.EAST;
					cannonType = 9;
					if (ammunition == -1) {
						ammunition = 2;
				...
				} else if (b.getRelative(BlockFace.SOUTH, 1).getType() == Material.WOOD
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.WEST;
					cannonType = 9;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.EAST, 1).getType() == Material.WOOD
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.SOUTH;
					cannonType = 9;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.WEST, 1).getType() == Material.WOOD
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.NORTH;
					cannonType = 9;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.NORTH, 1).getType() == Material.IRON_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.EAST;
					cannonType = 4;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.SOUTH, 1).getType() == Material.IRON_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.WEST;
					cannonType = 4;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.EAST, 1).getType() == Material.IRON_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.SOUTH;
					cannonType = 4;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.WEST, 1).getType() == Material.IRON_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.NORTH;
					cannonType = 4;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.NORTH, 1).getType() == Material.GOLD_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.EAST;
					cannonType = 5;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.SOUTH, 1).getType() == Material.GOLD_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.WEST;
					cannonType = 5;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.EAST, 1).getType() == Material.GOLD_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.SOUTH;
					cannonType = 5;
					if (ammunition == -1) {
				...
				} else if (b.getRelative(BlockFace.WEST, 1).getType() == Material.GOLD_BLOCK
						&& b.getRelative(BlockFace.DOWN, 1).getType() == Material.LAPIS_BLOCK) {
					direction = BlockFace.NORTH;
					cannonType = 5;
					if (ammunition == -1) {
		...
		if (direction != null) {
			...
			return true;
		} else // System.out.print("No Cannon");
			return false;
	}
	...
	public void turnCannonLayer(Boolean right, Player p, int offsetY) {
		...
		for (int x = 0; x < 7; x++) {
			for (int z = 0; z < 7; z++) {
				if (loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY).getTypeId() == 69
						|| loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY)
								.getTypeId() == 77) {
					loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY).setTypeIdAndData(0,
							(byte) 0, false);
				}
			}
		}
		...
		for (int x = 0; x < 7; x++) {
			for (int z = 0; z < 7; z++) {
				if (arro[x][z] != 69 && arro[x][z] != 77 && arro[x][z] != 23) {
					loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY)
							.setTypeIdAndData(arro[x][z], arrbo[x][z], false);
					if (testCraft != null) {
						testCraft.addBlock(
								loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY), true);
					}
		...
		if (offsetY == 0) {
			...
			Location teleLoc = new Location(p.getWorld(), loc.getBlock().getRelative(direction, -1).getX() + 0.5,
					loc.getBlock().getRelative(direction, -1).getY(),
					loc.getBlock().getRelative(direction, -1).getZ() + 0.5);
			...
			if (cannonTurnCounter < 4
					&& (loc.getBlock().getRelative(direction, 1).getRelative(BlockFace.DOWN, 1).getTypeId() == 5
							|| cannonType == 6 && loc.getBlock().getRelative(direction, 1)
									.getRelative(BlockFace.DOWN, 2).getTypeId() == 5)) {
				cannonTurnCounter++;
				turnCannon(right, p);
			} else {
				cannonTurnCounter = 0;
		...
		if (leftClick) {
			if (charged > 0) {
				...
				if (cannonType == 4) {
					...
					loc.getBlock().getRelative(direction, 4).getRelative(BlockFace.DOWN).setTypeIdAndData(35,
							(byte) 0x8, false);
					...
					loc.getBlock().getRelative(BlockFace.DOWN, 5).getRelative(BlockFace.DOWN).setTypeIdAndData(35,
							(byte) 0x7, false);
					fireDC(p, loc.getBlock().getRelative(BlockFace.DOWN, 5), 0,
							loc.getBlock().getRelative(BlockFace.DOWN, 5).getY(), 0, 2);
					...
					if (direction == BlockFace.NORTH || direction == BlockFace.SOUTH) {
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.EAST, 10).setTypeIdAndData(35,
								(byte) 0x8, false);
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.DOWN)
								.getRelative(BlockFace.EAST, 10).setTypeIdAndData(35, (byte) 0x8, false);
						fireDC(p, loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.EAST, 10), depth,
								loc.getBlockY(), 1000, 1);
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.WEST, 10).setTypeIdAndData(35,
								(byte) 0x8, false);
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.DOWN)
								.getRelative(BlockFace.WEST, 10).setTypeIdAndData(35, (byte) 0x8, false);
						fireDC(p, loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.WEST, 10), depth,
								loc.getBlockY(), 1000, -1);
					} else {
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.NORTH, 10).setTypeIdAndData(35,
								(byte) 0x8, false);
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.DOWN)
								.getRelative(BlockFace.NORTH, 10).setTypeIdAndData(35, (byte) 0x8, false);
						fireDC(p, loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.NORTH, 10), depth,
								loc.getBlockY(), 1000, 1);
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.SOUTH, 10).setTypeIdAndData(35,
								(byte) 0x8, false);
						loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.DOWN)
								.getRelative(BlockFace.SOUTH, 10).setTypeIdAndData(35, (byte) 0x8, false);
						fireDC(p, loc.getBlock().getRelative(direction, 6).getRelative(BlockFace.SOUTH, 10), depth,
								loc.getBlockY(), 1000, -1);
					}
	...
	public void fireDC(final Player p, final Block b, final int dcDepth, final int startY, final int delayShoot,
			final int direction) {
	...
	public void fireDCUpdate(final Player p, final Weapon dc, final int freeFallHt, final int i) {
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
			...
			if (dc.warhead.getTypeId() != 35) return;
			
			if (i >= dc.setDepth + freeFallHt || dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 8
					&& dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 9
					&& dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 0
					&& dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 79) {
				 ...
				 * if( breakChance == -1 )
				 * {
				 * TNTPrimed tnt = (TNTPrimed)theBlock.getWorld().spawnEntity(new Location(theBlock.getWorld(),
				 * theBlock.getX(), theBlock.getY(), theBlock.getZ()), EntityType.PRIMED_TNT);
				...
				if (checkCraft == null) {
					checkCraft = NavyCraft.instance.entityListener
							.structureUpdate(dc.warhead.getRelative(4, 4, 4).getLocation(), p);
					if (checkCraft == null) {
						checkCraft = NavyCraft.instance.entityListener
								.structureUpdate(dc.warhead.getRelative(-4, -4, -4).getLocation(), p);
						if (checkCraft == null) {
							checkCraft = NavyCraft.instance.entityListener
									.structureUpdate(dc.warhead.getRelative(2, -2, -2).getLocation(), p);
							if (checkCraft == null) {
								checkCraft = NavyCraft.instance.entityListener
										.structureUpdate(dc.warhead.getRelative(-2, 2, 2).getLocation(), p);
							}
		...
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, () -> {
			if (i == 0) {
				...
				if (num == 1 && !tntp.isDead()) {
					if (Math.signum(tnt1X) != Math.signum(tntp.getVelocity().getX())
							|| Math.signum(tnt1Z) != Math.signum(tntp.getVelocity().getZ())
							|| Math.abs(tntp.getVelocity().getX()) < Math.abs(tnt1X / 4)
							|| Math.abs(tntp.getVelocity().getZ()) < Math.abs(tnt1Z / 4)) {
						System.out.println("tnt1X=" + tnt1X + " tnt1Z=" + tnt1Z);
						System.out.println("tntp.getVelocity().getX()=" + tntp.getVelocity().getX()
								+ " tntp.getVelocity().getZ()=" + tntp.getVelocity().getZ());
						tntp.setFuseTicks(1);
					...
					if (tnt2X < tntp2.getVelocity().getX() - 0.5 || tnt2X > tntp2.getVelocity().getX() + 0.5
							|| tnt2Z < tntp2.getVelocity().getZ() - 0.5 || tnt2Z > tntp2.getVelocity().getZ() + 0.5) {
						tntp2.setFuseTicks(1);
					} else {
						tntp2.setFuseTicks(1000);
					}
				} else if (num == 3 && !tntp3.isDead()) {
					if (tnt3X < tntp3.getVelocity().getX() - 0.5 || tnt3X > tntp3.getVelocity().getX() + 0.5
							|| tnt3Z < tntp3.getVelocity().getZ() - 0.5 || tnt3Z > tntp3.getVelocity().getZ() + 0.5) {
